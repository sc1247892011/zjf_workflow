...
func (r *MySQLRepositoryService) SaveProcessDefinition(pd *ProcessDefinition) (int, error) {
	query := `
...
FROM 
    ...
    `
	result, err := r.DB.Exec(query, pd.ProcessDefinitionName, pd.XMLContent, pd.CreatedAt, pd.CreatedBy, pd.Status, pd.Description, pd.ProcessDefinitionName)
	if err != nil {
...
func (r *MySQLRepositoryService) GetProcessDefinitionById(id int) (*ProcessDefinition, error) {
	...
	err := r.DB.QueryRow(query, id).Scan(&pd.Id, &pd.ProcessDefinitionName, &pd.Version, &pd.XMLContent, &pd.CreatedAt, &pd.CreatedBy, &pd.Status, &pd.Description)
	if err != nil {
...
func (r *MySQLRepositoryService) GetProcessDefinitionByNameAndVersion(name string, version int) (*ProcessDefinition, error) {
	...
	pd := &ProcessDefinition{}
	err := r.DB.QueryRow(query, name, version).Scan(&pd.Id, &pd.ProcessDefinitionName, &pd.Version, &pd.XMLContent, &pd.CreatedAt, &pd.CreatedBy, &pd.Status, &pd.Description)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func (r *MySQLRepositoryService) GetLatestProcessDefinitionByName(name string) (*ProcessDefinition, error) {
...
FROM process_definition WHERE process_definition_name = pd.process_definition_name) AND pd.process_definition_name = ? `
	pd := &ProcessDefinition{}
	err := r.DB.QueryRow(query, name).Scan(&pd.Id, &pd.ProcessDefinitionName, &pd.Version, &pd.XMLContent, &pd.CreatedAt, &pd.CreatedBy, &pd.Status, &pd.Description)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func (r *MySQLRepositoryService) UpdateProcessDefinition(pd *ProcessDefinition) error {
	query := `
        UPDATE process_definition
        SET  xml_content = ?, created_by = ?
        WHERE id = ?
    `
	_, err := r.DB.Exec(query, pd.XMLContent, pd.CreatedBy, pd.Id)
	if err != nil {
...
func (r *MySQLRepositoryService) DeleteProcessDefinition(id int) error {
